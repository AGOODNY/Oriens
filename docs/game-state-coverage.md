...
- Python 状态现在保存当前房间内的 `collectible_spawned` 条目，并在换房时清空；`collectible_taken` 会标记对应条目已拾取。
- 无语音垂直切片只对 `room_type=4` 且本地固定资料已覆盖的道具生成建议，不推断未知道具。
- 当前固定测试 ID 为 `1`、`3`、`4`、`12`、`350`；资料与来源位于 `data/knowledge/items.zh-CN.json`。
- 画面读取、Modded 道具语义、完整交易约束、向量 RAG 仍不在阶段 1 范围内。
- 游戏画面与 Python 重建状态的逐项人工对照仍需在本轮实机验收中完成，不能由自动回放替代。

2026-08-10 首次阶段 1 对照中，资源与道具建议显示一致；房间原始类型值也与游戏枚举一致，但 UI 名称映射把 `ROOM_CURSE=10` 错写为恶魔/天使房，并遗漏 `ROOM_SECRET=7`、`ROOM_SUPERSECRET=8`。该问题属于展示层映射而非采集错误，已依据游戏随附 `resources/scripts/enums.lua` 修正。第二次实机核对确认刺房、隐藏房和超级隐藏房名称均正确，阶段 1 人工状态对照通过。
