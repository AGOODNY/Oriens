...
> 文档状态：阶段 4 本地长期记忆与可控的陪伴延续
> 更新日期：2026-08-14
> 当前阶段：阶段 3/3.6 功能保持可用；阶段 4 已在唯一应用组合根和桌面控制中心边界内实现默认关闭、来源可追溯、用户可查看/纠正/禁用/删除的本地长期记忆
...
阶段 4 不保存 API Key、业务空间 ID、令牌、授权头、原始麦克风录音、每帧游戏状态、完整聊天历史、无意义寒暄、未经验证的心理/身份推断、RAG 攻略事实或提示注入内容；不上传记忆，也没有实现完整陪伴人格。阶段 5 的视觉理解和阶段 6 的 Omni Realtime 仍未实现。
