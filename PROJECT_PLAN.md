...
> 文档状态：阶段 6 默认关闭、可随时降级的 Qwen Omni Realtime 实验模式
> 更新日期：2026-08-19
> 当前阶段：阶段 3/3.6/4/5 功能保持可用；阶段 6 已在唯一应用组合根内实现默认关闭、人工连接、主动提交语音、严格只读工具、预算护栏和链式降级的 Realtime 实验模式
...
目标：在不改变阶段 3 体验的前提下，为 Windows 桌面产品和阶段 4 建立稳定边界。

- `OriensApplication` 统一负责启动、服务装配和关闭；CLI 只提供开发与诊断入口。
- `OverlayWindow` 消费已装配的会话、RAG、模型和语音服务，不再选择基础设施实现。
- `AppPaths` 区分只读程序资源和配置、知识包、模型、缓存、日志、未来记忆等用户数据。
- `ConfigService` 支持发布默认、用户覆盖、显式 `--config` 三层优先级、schema 校验和原子保存；凭据不进入 TOML。
- 知识包采用独立 manifest、大小与 SHA-256 校验、路径穿越防护和先校验后启用；开发配置继续适配现有 `rag-v2.1`。
- `MemoryStore`、会话开始/结束、查询前召回和回答后候选提交位置已定义；当前使用不读写的 `NullMemoryStore`。
- 当前仍是开发启动方式；正式产品形态为桌面控制中心、后台核心、游戏悬浮窗，并预留系统托盘入口。

阶段 3.5 明确未实现长期记忆数据库、用户画像、陪伴人格、视觉理解、账号、云端代理或阶段 6 模型能力。

### 阶段3.6：最小 Windows 桌面产品外壳（已完成）

目标：不改变阶段 3 功能与阶段 3.5 架构边界，让普通用户通过一个桌面外壳控制共享后台核心与悬浮窗。

- 新增简体中文桌面控制中心，展示整体、监听、模式、配置、知识包、日志、RAG、语音、费用与长期记忆占位状态。
- `OriensApplication` 继续作为唯一组合根，并新增统一运行快照、日志轮询、暂停/恢复监听、共享建议任务池和幂等关闭。
- 控制中心、悬浮窗和系统托盘共享同一个游戏会话、预算、RAG、模型路由与语音服务，不重复装配后台核心。
- 关闭控制中心默认缩到托盘；关闭或隐藏悬浮窗不退出应用；只有“完全退出 Oriens”才关闭日志、任务、语音、音频、RAG Worker、托盘和全部 Qt 窗口。
- 托盘支持打开控制中心、显示/隐藏悬浮窗、暂停/恢复监听和完全退出；托盘不可用时保持控制中心可见并提供安全退出入口。
- 新增 `oriens desktop` 开发入口；原 `oriens ui` 参数与行为兼容并复用同一桌面启动接口。
- 设置页只保存现有白名单中的语音、按键说话、TTS 与预算字段；知识包选择走 `KnowledgePackManager`，均提示下次启动生效；不提供凭据输入。

阶段 3.6 仍不包含长期记忆业务、陪伴人格、视觉理解、Qwen Omni Realtime、账号、云同步、远程代理、自动更新、安装器或新的模型能力。当前仍是开发启动方式。

### 阶段4：本地长期记忆与可控的陪伴延续（已完成）

目标：Oriens能延续玩家关系，但不会胡乱记忆。

- `memory.enabled` 默认关闭并只允许通过用户设置白名单保存；禁用时使用 `NullMemoryStore`，不创建记忆目录或数据库。
- `OriensApplication` 只装配一个 `SQLiteMemoryStore`，数据库位于 `AppPaths.memory_dir`；控制中心、悬浮窗、文字问答与主动建议共享同一服务。
- schema 版本表、记忆主表和来源证据表使用外键、busy timeout 与显式事务；迁移失败不删除或重建旧库，并安全降级到无记忆模式。
- 保存玩家明确表达或手动确认的档案、稳定偏好、提示偏好和里程碑；候选经过类型/长度/敏感内容/置信度过滤、去重和冲突替换，低置信推断只进入待确认状态且不召回。
- 召回最多携带配置限定的少量字符，并以结构化不可信偏好数据传入；用户当前表达、当前游戏状态和本地 RAG 事实及引用始终优先。
- 控制中心可查看类型、内容、来源、时间和状态，并支持手动添加、纠正、单条启停、二次确认删除和二次确认清空。
- 关闭总开关在下次启动时生效，只停止写入与召回，不自动删除已有数据；单条禁用保留内容但不召回，删除与清空会真正删除内容及证据。
- 完全退出按既有生命周期取消交互与任务，结束当前记忆会话并幂等关闭 SQLite；数据库或迁移故障不阻止日志、RAG、文字问答、语音与悬浮窗。

验收：跨多次启动后能够正确回忆明确偏好，玩家可查看和删除全部记忆。

阶段 4 不保存 API Key、业务空间 ID、令牌、授权头、原始麦克风录音、每帧游戏状态、完整聊天历史、无意义寒暄、未经验证的心理/身份推断、RAG 攻略事实或提示注入内容；不上传记忆，也没有实现完整陪伴人格。阶段 5 的视觉补充现已独立完成，阶段 6 的 Omni Realtime 仍未实现。

### 阶段5：默认关闭、隐私优先的视觉补充（已完成）

目标：只为状态 API 与本地资料覆盖不到的场景增加按需截图理解。

- `vision.enabled` 默认关闭；禁用时装配 `NullVisionService`，不定位窗口、不截图、不创建调试目录、不调用视觉模型。
- 只接受用户点击“识别当前游戏画面”的人工触发；选择房、Modded 内容与非结构化 UI 是目标场景，不连续采集、不录屏。
- Win32 边界同时验证 `isaac-ng.exe`、游戏标题、可见性、非最小化、前台状态和客户区尺寸，只读取目标客户区；失败绝不回退全桌面。
- 商店、恶魔房和天使房裁掉外围 HUD，其余人工问题保留游戏客户区；裁剪、缩放、像素数和编码字节数均有边界。
- 截图默认只在内存中编码；单独开启调试保存后才写入 `AppPaths.vision_debug_dir`，普通 UI 不显示路径。
- 多模态图片通过兼容扩展后的 `ModelRequest` 与统一 `ModelRouter` 发送；模型 ID、端点、超时、重试和价格全部来自配置，纯文本调用保持兼容。
- 视觉输出经过字段、长度、置信度、状态序号、证据类型和场景一致性校验；低置信结果统一显示“无法可靠识别”。
- 视觉结果是独立补充证据，不进入结构化状态、RAG 引用或长期记忆，也不能覆盖这些事实。
- 唯一 `OriensApplication` 共享并关闭唯一视觉服务；新任务替换旧任务，任意受信状态变化、换房、用户取消和完全退出使旧结果失效。
- 独立限制请求间隔、单局次数和视觉费用；达到上限后安全退回文字问答。

自动验收使用合成图、模拟窗口与模拟模型完成，不访问真实桌面、真实游戏窗口或真实网络。真实 Windows 游戏窗口定位、DPI/多显示器画质、遮挡表现和北京地域视觉 API 仍列为显式授权后的人工验收项。

### 阶段6：实时语音实验（已完成模拟协议实现，待真实 API 人工验收）

目标：验证端到端语音是否值得作为高级模式。

- `realtime.enabled`、`semantic_vad_enabled` 与调试音频保存均默认关闭；禁用时只装配空实现，不连接、不打开麦克风、不创建线程或目录、不产生费用。
- `OriensApplication` 只装配一个独立 Realtime 会话服务；控制中心和悬浮窗只调用它，链式 `VoiceService` 始终保留且是默认/故障兜底。
- WebSocket 使用配置中的北京地域端点、模型、音色、PCM 格式、采样率、响应超时、重试、VAD 阈值、价格和会话阈值；Bearer 凭据不进入 URL、UI、用户 TOML 或日志。
- 默认仍为按键说话；输入 16 kHz 16-bit 单声道 PCM，输出复用 24 kHz PCM 有界播放。再次开口、新问题、取消、任意受信状态变化、换房和退出都会清空播放、发送适用取消并使旧转录/工具/文本/音频过期。
- 支持 `session.update`、音频追加/提交、转录增量、音频/文本增量、`response.done`、错误、取消与 Function Calling 回填；模型内置联网搜索固定关闭。
- 工具白名单仅有 `get_current_game_state`、`retrieve_local_rag`、`recall_confirmed_preferences`；严格校验 JSON、长度、数量、超时、重复 `call_id` 和状态序号，不提供 shell、文件、联网、记忆写入、截图或游戏控制。
- 独立费用统计优先使用 `response.done.usage`，缺失时按配置的保守 Token 率估算并标记；达到提醒阈值提示，达到硬上限取消会话并退回链式语音。
- 在配置的约 90 分钟、80 轮或 480 秒音频阈值主动轮换连接；只携带有界本地文本摘要，不保存原始音频或完整会话。关闭采用短时有界等待，不阻塞 Qt 等待官方可能需要 5–10 秒的关闭过程。
- 控制中心和悬浮窗显示模式、连接/聆听/思考/工具/说话/重连/降级状态、时长、轮次、估算费用、预算进度与 VAD，并提供连接、断开、取消和打断入口。
- `oriens realtime-benchmark` 使用相同模拟问题输出链式/Realtime 的首转录、首文本、首音频、打断、工具和总延迟以及队列、降级和估算费用；结果明确不代表真实网络体验。

2026-08-19 官方核对：模型别名为 `qwen3.5-omni-flash-realtime`（对应 2026-03-15 快照）；北京原价为文本/图片/视频输入 3.3、音频输入 27、纯文本输出 20、文本+音频输出 107 元/百万 Token。官方限制为单连接最长 120 分钟、约 80 轮/480 秒音频上下文，且联网搜索和工具调用不可同时启用。
