...
```text
请根据仓库根目录的 PROJECT_PLAN.md 初始化 Oriens: Your In-Game Guide 项目。

本轮只执行“阶段0：技术探针”，不要提前接入付费模型。请先检查工作区和开发环境，然后：
1. 初始化仓库和目录结构；
2. 创建最小可运行的以撒 Repentance+ Lua Mod；
3. 使用带 [ORIENS_EVENT] 前缀的单行 JSON 输出游戏事件；
4. 创建 Python 日志监听、事件 schema、状态聚合和日志回放的最小实现；
5. 编写基础测试和本地运行说明；
6. 验证后汇报游戏 API 能读取和不能读取的状态。

界面和文档默认使用简体中文。不要保存或提交任何API Key。遇到无法从本机自动确认的游戏路径或需要实际启动游戏验证的步骤时，明确告诉我需要我做什么。
```
